...
# @tool
# def get_country_report_2(tool_input, cat):
#     """Analyze the request from the user, and activate this function EVERYTIME the user asks information about any country or city. 
#     In order to usefully proceed with the analysis, if a city is mentioned, you have to convert the name of the city into the name of the related country. 
#     The final input for the script needs to be the country name. For example, if the user asks for reccomended vaccination for Lagos, you have to convert the request into vaccination for Nigeria. 
#     If the user asks for information about the italian embassy in Texas, you have to convert the request into the italian embassy in USA"""
...
The output is always None."""
    ...
    download_path = f'/app/cat/plugins/plugin-template/downloads/{tool_input}.pdf'
